...
function fifthExample()
    ...
    model = SDDP.LinearPolicyGraph(
        ...
        JuMP.@variable(linearizedSubproblem, 0.0 <= b <= 2.0, NCNBD.State, Bin, initial_value = 0)
    ...
    # WRITE LOGS TO FILE
    ############################################################################
    NCNBD.write_log_to_csv(model, "fifth_example.csv", algoParameters)

end

fifthExample()

# example related to discontExample but with binary state
